...
const ProjectSection = () => (
  <div id="project" className="mt-32">
    <div className="relative flex flex-col items-center mb-16" data-aos="fade-up" data-aos-duration="800" data-aos-once="true">
      <span className="absolute -top-8 left-1/2 -translate-x-1/2 text-[120px] font-black select-none pointer-events-none leading-none tracking-tighter" style={{ color: 'rgba(139,92,246,0.05)' }}>
        WORK
      </span>
      ...
      <div className="inline-flex items-center gap-2 mb-4 px-3 py-1.5 rounded-full" style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)' }}>
        ...
        <span className="text-xs font-semibold tracking-widest text-violet-600 uppercase">Selected Work</span>
        <span className="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse" />
      </div>

      <h2 className="text-5xl lg:text-6xl font-bold tracking-tight leading-tight text-center mb-4" style={{ color: '#1a1a2e' }}>
        My <span style={{ color: '#7c3aed' }}>Projects</span>
      </h2>
      ...
      <div className="flex items-center gap-8 mt-8" data-aos="fade-up" data-aos-duration="800" data-aos-delay="250" data-aos-once="true">
        {[
          ...
          <div key={i} className="flex flex-col items-center gap-1">
            <span className="text-xl font-bold" style={{ color: '#1a1a2e' }}>{num}</span>
        ...
        {listProyek.map((project, index) => (
          <div key={project.id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 border-l-4 hover:scale-105 transform" style={{ borderColor: project.borderColor, background: project.gradient ? `linear-gradient(145deg, ${project.borderColor}20, rgba(255,255,255,0.9))` : 'white' }}>
            <div className="flex items-center justify-between mb-4">
              ...
              {project.url && (
                <a href={project.url} target="_blank" rel="noopener noreferrer" className="text-violet-600 hover:text-violet-800 transition-colors">
            ...
            <h3 className="text-xl font-bold mb-3" style={{ color: '#1a1a2e' }}>{project.title}</h3>
